...
class Point(object):
    ...
    def move_by(self, dx, dy):
        x_distance = abs(self.x - (self.x + dx))
        y_distance = abs(self.y - (self.y + dy))
        distance = math.sqrt((x_distance ** 2) + (y_distance ** 2))
        self.distraveled += distance
        self.x = self.x + dx
        self.y = self.y + dy
        self.count += 1
    ...
    def get_distance_from(self, point2):
        x_distance = abs(self.x - point2.x)
        y_distance = abs(self.y - point2.y)
        distance = math.sqrt((x_distance ** 2) + (y_distance ** 2))
        return distance
